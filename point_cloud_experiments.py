...
if __name__ == "__main__":
    ...
    for n_repeat in range(n_runs):
        ...
        ## datasets
        #datasets = ['Orbit5k', 'ModelNet40_512_points']
        datasets = ['Orbit5k']
        for dataset_name in datasets:
            ...
            # define our model    
            name = 'size_pred_pointnet_' + dataset_name
            size_predictor = models.point_clouds_encoders.PointNet(emb_dims=128, 
                                                                   dim_in=in_channels, 
                                                                   output_channels=1).to(device)
            
            size_predictor = train_models.train_size_predictor(size_predictor, dataloader_train, dataloader_test, name)

            # add other generators and encoders
            latent_dim = 1024
            point_net = models.point_clouds_encoders.PointNet(emb_dims=256, 
                                                              dim_in=in_channels, 
                                                              output_channels=latent_dim).to(device)
            ...
            dgcnn = models.point_clouds_encoders.DGCNN(k=10, 
                                                       dim_in=in_channels, 
                                                       emb_dims=256, 
                                                       dropout=0.1, 
                                                       output_channels=50*50).to(device)

            point_cloud_pi_models = [(topology_net, 'TopologyNet'), 
                                     (ripsnet, 'RipsNet'), 
                                     (dgcnn, 'DGCNN')]

            for pi_model, pi_model_name in point_cloud_pi_models:
                ...
                final_metrics[name + '_logreg_acc'].append(acc_logreg)
                final_metrics[name + '_rfc_acc'].append(acc_rfc)
            path_cur = './results/temp_pc_run_{}.pt'.format(n_repeat)
