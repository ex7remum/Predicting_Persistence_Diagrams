...
import math
from models import TransformerLayer, MLP, create_padding_mask, MultiHeadAttention
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, X, mask=None):
        ...
        if mask is None:
            mask = torch.ones((X.shape[0], X.shape[1])).to(torch.long).to(X.device)
        # attention_scores = []
        ...
        if self.reduction == "mean":
            ...
            lengths = mask.sum(dim=1).detach()
            outputs = (outputs * mask.unsqueeze(2)).sum(dim=1)
        else:
...
class PointNet(nn.Module):
    def __init__(self, emb_dims=256, dim_in=3, output_channels=40):
